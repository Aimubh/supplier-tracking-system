...
// StaggeredMenu (React Bits) — a full-screen slide-out overlay menu with a
// hamburger toggle and staggered GSAP entrance. Used as the MOBILE navigation
// (the desktop sidebar is hidden below lg). Adapted to TypeScript and to close
// on navigation. On-brand ink/paper palette.
...
export function StaggeredMenu({
  ...
  React.useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (panelRef.current && !panelRef.current.contains(t) && toggleBtnRef.current && !toggleBtnRef.current.contains(t)) {
        closeMenu();
      }
    };
            ...
            {items.length ? (
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  {it.disabled ? (
                    <span className="sm-panel-item sm-panel-item--disabled" aria-label={it.ariaLabel} data-index={idx + 1}>
                    ...
                    <a
                      ...
                      data-index={idx + 1}
                      onClick={(e) => {
                        // Client-side nav via the callback; close the menu.
                        if (onItemClick) { e.preventDefault(); onItemClick(it.link); }
                        closeMenu();
                      }}
                    >
                      <span className="sm-panel-itemLabel">{it.label}</span>
